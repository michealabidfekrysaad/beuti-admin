  ...
  const checkAllOptionsUnderService = (catIndex, serIndex, optionsArray) => {
    optionsArray.forEach((element, index) =>
      setValue(`servicesOptions[${catIndex}].services[${serIndex}].options[${index}]`, {
        serviceOptionID: element.serviceOptionID.toString(),
        count:
    ...
    services[catIndex].categoryServiceDto.forEach((element, index) =>
      element.options.map((el, optIndex) =>
        setValue(`servicesOptions[${catIndex}].services[${index}].options[${optIndex}]`, {
          serviceOptionID: el.serviceOptionID.toString(),
          count:
      ...
      el.categoryServiceDto.map((single, indxSer) =>
        single.options.map((opt, indxOpt) =>
          setValue(
            ...
            {
              serviceOptionID: opt?.serviceOptionID?.toString(),
              count:
    ...
    >
      <Modal.Body className="categoryModal">
        ...
        <div className="categoryModal-subtitle">
          {messages['offers.syb.title.select.service.modal']}
        </div>
        {!searchValue && (
          <Col className="mb-3">
                                  ...
                                  service.options.map((serOption, optionIndex) => (
                                    <span className="mx-1">
                                      <FormControlLabel
                                        ...
                                        label=""
                                        value={serOption?.serviceOptionID}
                                        {...register(
                                          `servicesOptions[${catIndex}].services[${serIndex}].options[${optionIndex}].serviceOptionID`,
                                        )}
                                      />
                                      {/* buttons removed to increase deacrease services number */}
                                    </span>
                                  ))}
                              </Col>
                              {/* options checkboxes */}
                                ...
                                service.options.map((serOption, optionIndex) => (
                                  <Col xs={12} className="mx-5 my-2">
                                    <FormControlLabel
                                      ...
                                      label={
                                        serOption?.name || serOption?.serviceOptionID
                                      }
                                      value={serOption?.serviceOptionID}
                                      {...register(
                                        `servicesOptions[${catIndex}].services[${serIndex}].options[${optionIndex}].serviceOptionID`,
                                    ...
                                    />
                                    {/* buttons remove to increase deacrease services number */}
                                  </Col>
                                ))}
                            </Row>
                          </Col>
...
ServiceAddModalOffer.propTypes = {
  show: PropTypes.bool,
...
export default ServiceAddModalOffer;
