...
import { SERVICE_PROVIDER_SERVICES_AUTO_COMPLETE } from 'utils/API/EndPoints/ServiceProviderEP';
import { toast } from 'react-toastify';

export default function ServiceAutoCompelete({
  handleEnNameInput,
  handleArNameInput,
  nameAr,
  useFormRef,
  ...
  const { refetch, isFetching: fetchServices } = CallAPI({
    name: 'autoCompleteServiceSearch',
    url: SERVICE_PROVIDER_SERVICES_AUTO_COMPLETE,
      ...
      renderInput={(params) => (
        <>
          <TextField
            {...params}
            {...useFormRef}
            label={`${messages['wizard.add.new.service.ar.name']} *`}
            InputLabelProps={{
              shrink: false,
            }}
            //   onChange={(e) => {
            //     handleArNameInput(e.target.value);
...
ServiceAutoCompelete.propTypes = {
