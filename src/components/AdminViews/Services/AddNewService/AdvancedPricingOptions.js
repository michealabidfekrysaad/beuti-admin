...
export default function AdvancedPricingOptions({
  advancedPriceModal,
  ...
  const empWithBranchThatSelected = watch('branchesCheckboxes');
  ...
  const putDefaultDurationForPriceOrBranch = () => {
    if (
      !durationService.find(
        (el) =>
          el?.id ===
          getValues(
            `pricing[${indexOfPricing}].employeePriceOptions[${branchesWithEmpSelectedMainPage?.findIndex(
              (br) => +br?.branchID === +idBranchSelectDD,
            )}].duration`,
          ),
      )
    ) {
      return durationService.find(
  ...
  const returnIDWithDefaultOrNot = () => {
    if (
      getValues(
        `pricing[${indexOfPricing}].employeePriceOptions[${branchesWithEmpSelectedMainPage?.findIndex(
          (br) => +br?.branchID === +idBranchSelectDD,
  ...
  const returnIDTypePriceWithDefaultOrNot = () => {
    if (
      getValues(
        `pricing[${indexOfPricing}].employeePriceOptions[${branchesWithEmpSelectedMainPage?.findIndex(
          (br) => +br?.branchID === +idBranchSelectDD,
  ...
  const putDefaultPriceTypeForPriceOrBranch = () => {
    if (
      !dropDownOption.find(
        (el) =>
          el?.id ===
  ...
  const specialDurationEmp = [
    {
      key: -1,
      text: `${durationService.find(
        (el) => el?.id === getValues(`pricing[${indexOfPricing}].duration`),
      ) && putDefaultDurationForPriceOrBranch()}  ${messages['advance.default']}`,
      id: returnIDWithDefaultOrNot(),
    },
    ...durationService,
  ];

  const specialDropDownOptionEmp = [
    {
      index: -1,
      key: -1,
      id: returnIDTypePriceWithDefaultOrNot(),
      text: `${dropDownOption.find(
        (el) => el?.id === getValues(`pricing[${indexOfPricing}].priceType`),
      ) && putDefaultPriceTypeForPriceOrBranch()}  ${messages['advance.default']}`,
    },
    ...dropDownOption,
  ];
  ...
  useEffect(() => {
    if (branchesDD?.length) {
      setIdBranchSelectDD(branchesDD[0]?.id);
    }
  }, [branchesDD?.length]);
  ...
  const showDefaultsOrChoosenByUserForEmp = (
    indexOfPrice,
    indexOfEmp,
    optName,
    empIndex = false,
    ...
    // work with employee inputs
    if (!empOptName && indexOfPrice >= 0) {
      if (branchOptName?.includes('Default')) {
        return mainPriceOptName;
      }
      if (!branchOptName?.includes('undefined')) {
        return `${branchOptName}Default`;
      }
      return mainPriceOptName;
    }
    if (empOptName) {
  ...
  const checkIfNoDataInBranchPriceOrPriceWithVatToReturnPriceForMain = (
    indexOfPrice,
    indexOfBranch,
    optName,
  ) => {
    if (
      !getValues(
        `pricing[${indexOfPrice}].employeePriceOptions[${indexOfBranch}].${optName}`,
      )
    ) {
      return getValues(`pricing[${indexOfPrice}].${optName}`);
    }
    return getValues(
      `pricing[${indexOfPrice}].employeePriceOptions[${indexOfBranch}].${optName}`,
    );
                ...
                messages[
                  `${
                    allBranches?.length > 1
                      ? 'advance.price.options.emp.branch.header'
                      : 'advance.price.options.emp.header'
                  }`
                ]
              }
            </Col>
            {allBranches?.length > 1 && branchesWithEmpSelectedMainPage?.length > 0 && (
              <Col xs={3} className="mb-2">
                <SelectInputMUI
                  label={messages['advance.price.branch']}
                  list={branchesDD}
                  defaultValue={idBranchSelectDD}
                  value={idBranchSelectDD}
                  onChange={(e) => setIdBranchSelectDD(e.target.value)}
                />
              </Col>
            )}
          </Row>
                ...
                {allBranches?.length > 1 &&
                  (branchesWithEmpSelectedMainPage?.length > 0 ? (
                    branchesWithEmpSelectedMainPage.map(
                      (
                        el,
                        index = branchesWithEmpSelectedMainPage?.findIndex(
                          (br) => +br?.branchID === +idBranchSelectDD,
                        ),
                      ) => (
                        <Col
                        ...
                        >
                          <Row>
                            <Col xs={12} className="advPriceModal-body_row-header pb-1">
                              {messages['advance.price.per.branch']}
                            </Col>
                            <Col
                              xs={4}
                              className="advPriceModal-body_row-blueSection_branchDetails"
                            >
                              <img
                                src={
                                  getImageOfSelectedBranch(el?.branchID) || noBranchImg
                                }
                                alt={getNameOfSelectedBranch(el?.branchID)}
                                width="80"
                                height="80"
                                className="rounded"
                                onError={(e) => {
                              ...
                              <div className="branch-info">
                                <p className="header">
                                  {getNameOfSelectedBranch(el?.branchID)}
                                </p>
                                <p className="details">
                                  {getAddressOfSelectedBranch(el?.branchID)}
                                </p>
                              </div>
                            </Col>
                            <Col xs={2}>
                              <SelectInputMUI
                                useFormRef={register(
                                  `pricing[${indexOfPricing}].employeePriceOptions[${index}].duration`,
                                )}
                                watch={watch}
                                error={
                                  errors?.pricing &&
                            ...
                            <Col xs={2}>
                              <SelectInputMUI
                                useFormRef={register(
                                  `pricing[${indexOfPricing}].employeePriceOptions[${index}].priceType`,
                                )}
                                watch={watch}
                                error={
                                  errors?.pricing &&
                              ...
                              {getValues(
                                ...
                                !getValues(
                                  ...
                                  <BeutiInput
                                    ...
                                    info={messages['common.currency']}
                                    useFormRef={register(
                                      `pricing[${indexOfPricing}].employeePriceOptions[${index}].price`,
                                    )}
                                    error={
                                      errors?.pricing &&
                                      errors?.pricing[indexOfPricing] &&
                                      ...
                                      errors?.pricing[indexOfPricing]
                                        ?.employeePriceOptions[index]?.price?.message
                                    }
                              ...
                              {vatFromBE > 0 &&
                                ...
                                !getValues(
                                  ...
                                  <BeutiInput
                                    label={messages['newService.price.with.vat']}
                                    useFormRef={register(
                                      `pricing[${indexOfPricing}].employeePriceOptions[${index}].priceWithVat`,
                                    )}
                                    info={messages['common.currency']}
                                    // disabled
                                    readOnly
                                    error={
                                      ...
                                      errors?.pricing[indexOfPricing]
                                        ?.employeePriceOptions &&
                                      errors?.pricing[indexOfPricing]
                                        ?.employeePriceOptions[index]?.priceWithVat
                                        ?.message
                                    }
                                    placeholder={getValues(
                                ...
                                >
                                  ...
                                  <Col xs={2}>
                                    <SelectInputMUI
                                      ...
                                      label={messages['newService.duration']}
                                      list={specialDurationEmp}
                                      defaultValue={showDefaultsOrChoosenByUserForEmp(
                                        indexOfPricing,
                                        index,
                                        'duration',
                                        empIn,
                                      ...
                                      className={`${showDefaultsOrChoosenByUserForEmp(
                                        indexOfPricing,
                                        index,
                                        'duration',
                                        empIn,
                                    ...
                                    {getValues(
                                      ...
                                      !getValues(
                                        ...
                                        <BeutiInput
                                          ...
                                          placeholder={checkIfNoDataInBranchPriceOrPriceWithVatToReturnPriceForMain(
                                            indexOfPricing,
                                            index,
                                            'price',
                                          )}
                  ...
                  (branchesWithEmpSelectedMainPage?.length > 0 ? (
                    branchesWithEmpSelectedMainPage.map(
                      (
                        ...
                        >
                          <Row>
                            {/* below sectoin disaapear when one branch */}
                            <Col
                              xs={12}
                              className="advPriceModal-body_row-header pb-1 d-none"
                            >
                              {messages['advance.price.per.branch']}
                            </Col>
                            <Col
                              xs={4}
                              className="advPriceModal-body_row-blueSection_branchDetails d-none"
                            >
                              <img
                                src={
                                  getImageOfSelectedBranch(el?.branchID) || noBranchImg
                            ...
                            <hr className="w-100 my-4 d-none" />
                            {/* above sectoin disaapear when one branch */}
                            <Col xs={12} className="advPriceModal-body_row-header pb-1">
                              {messages['advance.price.per.emp']}
                            </Col>
                            {el.employees?.length > 0 ? (
...
AdvancedPricingOptions.propTypes = {
