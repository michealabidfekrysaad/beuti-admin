...
export default function PackageServices({
  ...
  const serPackageComponentRef = useRef(null);
  useEffect(() => {
    if (
      ...
      window.scrollTo(0, serPackageComponentRef?.current?.offsetTop);
    }
  }, [errors]);
  ...
  const merge = (serSelected, allPricingOptions) =>
    serSelected?.map((el) => {
      const fo = allPricingOptions.find(
        (element) => +element?.priceOptionID === +el?.serviceOptionID,
      );
      return {
        ...fo,
        count: el?.count,
  ...
  const calculateTotalDuration = (alloptions) => {
    ...
    let minutes = 0;
    servicesName(alloptions)?.forEach((element) => {
      hours += +element.duration.split(':')[0] * element?.count;
      minutes += +element.duration.split(':')[1] * element?.count;
    });
    if (minutes >= 60) {
  ...
  return (
    <Row className="pt-3" ref={serPackageComponentRef}>
      <Col xs={12} className="addPackage-title">
        {messages['package.service.title']}
      </Col>
      <Col xs={12} className="addPackage-subTitle pb-3">
        {messages['package.service.sub.title']}
      </Col>
              ...
              <Table className="mt-2">
                <TableBody className="serBody">
                  {servicesName(arrayOfOptions)?.map((el) => (
                    <>
                      <TableRow className="serBody-row" key={el?.priceOptionID}>
                        <TableCell className="serBody-row_countName">
                          <p className="serBody-row_countName-count">
                            <span>{el?.count}</span>{' '}
                            <i className="flaticon2-cancel-music"></i>{' '}
                          </p>
                          <span className="serBody-row_countName-name">
                            {getOptionNameOrServiceName(el?.priceOptionID)}
                          </span>
                        </TableCell>
                        ...
                        <TableCell className="serBody-row_priceDelete">
                          ...
                          <button
                            type="button"
                            className="btn"
                            onClick={() => removeService(el?.priceOptionID)}
                          >
