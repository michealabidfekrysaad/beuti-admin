...
import { getDDTimeByValue, handleCasesOfNewShiftTime } from '../../../../constants/hours';
...
const AddDeleteShift = ({ shiftLength, watch, dayPath, setValue, currentShift }) => {
  ...
  const handleAddShift = () => {
    ...
    const [secondShiftStartTime, secondShiftEndTime] = handleCasesOfNewShiftTime(
      getDDTimeByValue(dayShifts[0].startTime, locale).key,
      getDDTimeByValue(dayShifts[0].endTime, locale).key,
      locale,
    );
    const newShifts = [
      ...dayShifts,
